...
# These are the values in which kicks in speed up mode.
# Tested are 3, 4, 5
...
# Using classifier to recognize face
        ...
        while not should_stop:
            ...
            if self.send_rc_control and not OVERRIDE:
                for (x, y, w, h) in faces:
                    ...
                    # Position adjustment in X, Y, Z axis
                    if not args.debug:
                        # for turning
                        if vDistance[0] < -szX:
                            self.yaw_velocity = S
            ...
            # Shows what mode the drone is in and which distnace preset the drone is following.
            if OVERRIDE:
                show = "OVERRIDE: {}".format(oSpeed)
                dCol = (255,255,255)
            else:
                show = "AI mode: {}".format(str(tDistance))
